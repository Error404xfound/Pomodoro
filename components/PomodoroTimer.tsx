...
export default function PomodoroTimer() {
	...
	useEffect(() => {
		...
		const timerId = setInterval(() => {
			setCurrentTime((prevTime) => {
				...
				const nextMode: TimerMode = previousMode === "focus" ? "break" : "focus";
				...
				const nextTime = getDurationByMode(nextMode, focusLength, breakLength) * 60;
				return nextTime;
			});
		}, 1000);
	...
	const increase = useCallback((mode: TimerMode) => {
		if (isRunning) {
			return;
		}
	...
	const decrease = useCallback((mode: TimerMode) => {
		if (isRunning) {
			return;
		}
	...
	return (
		...
		>
			...
			>
				...
				<fieldset className="mx-auto w-full max-w-md" aria-label="Session mode selection">
					<legend className="mb-2 text-sm font-semibold text-zinc-800">Session Mode</legend>
					<div className="flex rounded-lg border border-zinc-300 bg-white p-1" role="group" aria-label="Mode switcher">
						<button
							type="button"
			...
			>
				<legend className="px-1 text-sm font-semibold text-zinc-800">Reward Effects</legend>
				...
				{prefersReducedMotion ? (
					<p className="mt-3 text-xs text-zinc-700">
						Animations are automatically reduced because your system preference requests reduced motion.
					</p>
				) : null}
			</fieldset>
